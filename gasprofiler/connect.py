"""Native cognis-connect emit for gasprofiler — forward findings to any platform.

Maps gasprofiler's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    gasprofiler ... --format json | gasprofiler-emit --to stix
    gasprofiler-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('function', '') + ' - ' + rec.get('opcode', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'gas')
